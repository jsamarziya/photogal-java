...
<fmt:message var="dateFormat"
	key="${(command.allMonths || (command.month != null))? 'format.date.imagesByDate' : 'format.date.imagesByUnknownDate'}" />
<pg:formatCalendarDate pattern="${dateFormat}"
	value="${command.calendarDate}" var="formattedDate" />
<div class="title"><fmt:message
	key="body.title.imagesByDate.${command.dateType}">
...
<c:url var="pageURL" value="/showImagesByDate.do">
	<c:param name="year" value="${command.year}" />
	<c:param name="month" value="${command.month}" />
	<c:param name="dateType" value="${command.dateType}" />
	<c:param name="allMonths" value="${command.allMonths}" />
</c:url>
