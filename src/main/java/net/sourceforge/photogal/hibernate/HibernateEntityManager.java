...
public class HibernateEntityManager {
     ...
     * 
     * @param includePrivate if <code>true</code>, return private galleries
     * too
    ...
    public ImageDescriptor getImageDescriptor(final long imageId) {
        ImageDescriptor retval = (ImageDescriptor) getSession()
                .createCriteria(ImageDescriptor.class)
                .add(Restrictions.eq("id", imageId)).uniqueResult();
        return retval;
    }
    ...
    public int getImageGalleryCount(long imageId) {
        int retval = (Integer) getSession()
                .getNamedQuery("getImageGalleryCount").setLong("imageId",
                                                               imageId)
                .uniqueResult();
    ...
    public void deleteGallery(final long galleryId, boolean deleteOrphanImages) {
        ...
        List<Gallery> list = getSession().createCriteria(Gallery.class)
                .add(Restrictions.gt("orderIndex", gallery.getOrderIndex()))
                .list();
        for (Gallery g : list) {
    ...
    private void moveGalleryDown(final int fromIndex, final int toIndex) {
        final Gallery gallery = (Gallery) getSession()
                .createCriteria(Gallery.class).add(
                                                   Restrictions
                                                           .eq("orderIndex",
                                                               fromIndex))
                .uniqueResult();
        List<Gallery> list = getSession().createCriteria(Gallery.class)
                .add(Restrictions.gt("orderIndex", fromIndex))
                .add(Restrictions.le("orderIndex", toIndex)).list();
        for (Gallery g : list) {
            final int oldIndex = g.getOrderIndex();
            final int newIndex = oldIndex - 1;
            log.debug("moving gallery#" + g.getId() + " from " + oldIndex
    ...
    private void moveGalleryUp(final int fromIndex, final int toIndex) {
        ...
        final List<Gallery> list = getSession().createCriteria(Gallery.class)
                .add(Restrictions.ge("orderIndex", toIndex))
                .add(Restrictions.lt("orderIndex", fromIndex)).list();
        for (Gallery g : list) {
            final int oldIndex = g.getOrderIndex();
            final int newIndex = oldIndex + 1;
            log.debug("moving gallery#" + g.getId() + " from " + oldIndex
     ...
     * 
     * @param includePrivate if <code>true</code>, return keywords for
     * private images too
    ...
    public Map<String, Integer> getKeywords(boolean includePrivate) {
        final List<String> keywords = getSession()
                .getNamedQuery(
                               includePrivate ? "getAllKeywords"
                                       : "getPublicKeywords").list();
    ...
    public Map<Long, CalendarDate> getImageCreationDates(
        ...
        final List<Object[]> results = getSession()
                .getNamedQuery(
        ...
        final List<Object[]> results = getSession()
                .getNamedQuery(
                               includePrivate ? "getAllDescriptorCreationDates"
                                       : "getPublicDescriptorCreationDates")
                .list();
        for (Object[] r : results) {
